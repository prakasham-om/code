...
try {
const res = await axios.get("[https://code-3oqu.onrender.com/api/admin/summary](https://code-3oqu.onrender.com/api/admin/summary)");
setSummary(res.data);
} catch (err) {
...
return ( <div className="p-4 sm:p-6 bg-gray-50 min-h-screen text-gray-800"> <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6"> <h1 className="text-2xl font-bold">📊 Admin Dashboard</h1> <div className="flex flex-col sm:flex-row gap-3">
\<input
type="date"
value={selectedDate}
        ...
        <tbody className="divide-y divide-gray-200">
          {users.map((u) =>
            (u.files || []).map((f, idx) => (
              <tr
                key={`${u._id}-${idx}`}
                className={`hover:bg-gray-50 ${!f.adminFileUrl ? "bg-blue-50" : ""}`}
              >
